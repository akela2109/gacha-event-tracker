...
- ↗ **Поделиться**: кнопка в шапке — на телефоне открывает системное окно «Поделиться»,
  на ПК показывает поповер с короткой ссылкой (копировать) и QR-кодом (`qr.png`).
  Короткая ссылка: **https://is.gd/ee6QPc** → ведёт на сайт.
...
│   └── share.js            # кнопка «Поделиться»: nav.share / копирование / QR
└── data/
